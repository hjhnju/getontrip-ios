...
class HttpRequest {
    ...
    typealias RequestDataCallBack = (result: NSData?, status: Int) -> ()
    ...
    /**
     ...
     - parameter handler: 回调数据及错误
     */
    class func ajax3(url: String?, path: String?, handler: RequestDataCallBack) {
        ...
        if let cacheJson = Cache.shareInstance.getObjString(urlPath) {
            //先看有效缓存，有直接返回
            if let data = cacheJson as? NSData {
                handler(result: data, status: RetCode.SUCCESS)
                hasValidCache = true
            }
        } else if let cacheJson = Cache.shareInstance.getObjString(urlPath) {
            //无则看有没缓存先用于展示
            if let data = cacheJson as? NSData {
                handler(result: data, status: RetCode.SUCCESS)
                hasDisplayCache = true
        ...
        HttpRequest.sharedManager.download(.GET, url, destination: destination).progress { (bytesRead, totalBytesRead, totalBytesExpectedToRead) -> Void in
...
//                print("Total bytes read on main queue: \(totalBytesRead)")
            ...
            if let data = data {
                Cache.shareInstance.setObjString(urlPath, value: data)
            }
        }
